...
function fillItem($item, itemData)
{
    ...
    if(itemData.acId)
    {
        ...
        if(index != -1)
        {
            $item.find('.relate-gift').show().on('click', function()
            {
                location.href = $.htmlRoot + "gift_detail.html" +  "?acId=" + itemData.acId + "&num=" + myGiftData[index].giftNo;
...
//                );
        ...
        else
        {
            $item.find('.relate-gift').show().on('click', function()
            {
                location.href = $.htmlRoot + "gift_detail.html" +  "?acId=" + itemData.acId;
//                isUxbao && window.uxbao.click(JSON.stringify
//                    (
//                        {
//                            "type":12,
...
//                );
    ...
    {
        $item.find('.relate-gift').hide().off('click');
    }
...
function btnClickHandler($target)
{
    ...
    doDownload($item.data("name"));
    location.href = $item.data("location");
    //通知android下载
