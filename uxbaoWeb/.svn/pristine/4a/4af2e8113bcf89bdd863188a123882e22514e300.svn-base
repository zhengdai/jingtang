...
//            );
    ...
    $('.header-grade').find('img').each(function(j, imgItem)
    {
        if(j - rated < -0.5)
        {
...
//进入游戏详情
function infoClickHandler($info)
{
    var resId = $info.data("id");
    location.href = ajaxGameDetail.gameDetailUrl + "?resId=" + resId;
    ...
    $("#tip-dialog").dialog({
        ...
        buttons: {
            ...
            "确定": function()
            {
                this.close();
                doDownload("游戏宝");
                location.href = 'http://q.gambao.com/Uxbao.apk';
    ...
    $.ajax(
        {
            ...
            success: function (data)
            {
                if(data.state == 1)//获取成功
                {
                    ...
                    $('.tit, .game-tit, #game-name').text(data.product.resName);
                    var ca = (data.product.resCapacity/(1024 * 1024)).toFixed(1);
                    $('.company').text(ca + 'MB | ' + data.product.resDeveloper);
                    $('.down-btn').on('click', function()
                    {
                        doDownload(data.product.resName);
                        location.href = data.product.resLocation;
                    });
                    //礼包
                        ...
                        $(activityList).each(function(i, activity)
                        {
                            ...
                            $giftTpl.on('click', function()
                            {
                                ...
                                if(activity.acType === 1)
                                {
                                    ...
                                    if(i != -1)
                                    {
                                        location.href = ajaxGameDetail.giftUrl +  "?acId=" + activity.acId + "&num=" + myGiftData[index].giftNo;
...
//                                        );
                                    ...
                                    {
                                        location.href = ajaxGameDetail.giftUrl +  "?acId=" + activity.acId;
//                                        isUxbao && window.uxbao.click(JSON.stringify
//                                            (
//                                                {
//                                                    "type":12,
...
//                                    );
                    ...
                    if(data.news)
                    {
                        ...
                        $(newsList).each(function(i, newsData)
                        {
                            ...
                            $newsTpl.on('click', function ()
                            {
                                location.href = ajaxGameDetail.infoUrl +  "?infoId=" + newsData.infoId;
//                                isUxbao && window.uxbao.click(JSON.stringify
//                                    (
//                                        {
//                                            "type":14,
