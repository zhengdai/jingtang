//滚动图片构造函数
function ScrollPic(scrollContId,arrLeftId,arrRightId,dotListId,itemClass,listType)
{
	...
	this.itemClass = itemClass;//表项的class
	...
	this.autoPlayTime = 8; //秒
  	...
  	this._autoTimeObj;
  	this._scrollTimeObj;
  	this._state = "ready"; // ready | floating | stopping
  
  
  	this.stripDiv = document.createElement("DIV");//在slide_01下
...
ScrollPic.prototype = 
{
  	...
  	{
  	    //初始化
        var pages = $("." + this.itemClass).length,i,tempObj;
        ...
        $("." + this.itemClass).width(this.pageWidth);
        //Arrowhead event
        //left
        if(this.arrLeftId)
        {
    ...
    readCookie : function(l)
    {
        ...
        if(document.cookie.length>0)
        {
            ...
            if(offset!=-1)
            {
                ...
                if(end==-1)
                {
                    end=document.cookie.length;i=unescape(document.cookie.substring(offset,end));
                }
            }
        }
        return i;
